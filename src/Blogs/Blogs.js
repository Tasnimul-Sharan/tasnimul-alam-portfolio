...
const Blogs = () => {
  const [openQuestions, setOpenQuestions] = useState(Array(8).fill(false));
  ...
  const isOpen = (index) => {
    return openQuestions[index];
  };

  return (
    <section className="py-16 bg-gradient-to-r from-indigo-900 to-purple-700 px-4 relative overflow-hidden">
      <div className="container mx-auto flex flex-col md:flex-row items-center gap-8">
        <div className="md:w-2/4 py-20 text-center md:text-left text-gray-100">
          <div className="space-y-4">
            <h2 className="text-5xl font-bold">
              Frequently Asked Questions
            </h2>
            <p className="text-lg">
              Here are some common questions we receive from clients and our responses.
            </p>
          </div>
        </div>
        ...
        <div className="md:w-3/4 md:mb-0">
          <div className="space-y-6 text-white">
            {[
              ...
              "How Do You Ensure Good User Experience (UX)?",
              "How Do You Handle Project Timelines and Communication?",
              "What Is Your Approach to Performance Optimization?",
              "Do You Implement SEO Best Practices?",
              "How Do You Handle Security?",
            ].map((question, index) => (
              <div
                key={index}
                className={`border ${
                  isOpen(index) ? "border-yellow-500" : "border-gray-200"
                } p-4 rounded-lg`}
              >
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-medium">{question}</h3>
                  <button
                    className={`text-white border rounded-full bg-${
                      isOpen(index) ? "yellow" : "purple"
                    }-800 hover:bg-${
                      isOpen(index) ? "yellow" : "purple"
                    }-900 focus:outline-none`}
                    onClick={() => toggleAnswer(index)}
                  >
                    {isOpen(index) ? <BsDash size={30} /> : <BsPlus size={30} />}
                  </button>
                </div>
                {isOpen(index) && (
                  <p className="mt-2 text-gray-100">
                    {index === 0
                      ? "I specialize in [React/Angular/Vue/Svelte/HTML/CSS/JavaScript], depending on the project's needs. I also use supporting libraries and tools like Tailwind CSS, Redux, and Axios for enhanced functionality and efficient development."
                      : index === 1
                      ? "I ensure cross-browser compatibility by using best practices like responsive design, CSS resets, and thorough testing. I test websites on major browsers like Chrome, Firefox, Safari, and Edge, as well as on mobile devices."
                      : index === 2
                      ? "I follow a mobile-first approach, using responsive frameworks like Bootstrap or custom media queries to ensure the website looks and functions well across a range of screen sizes."
                      : index === 3
                      ? "I focus on intuitive navigation, clean design, and fast load times. I also incorporate user feedback and testing to continuously improve the user experience. Accessibility is a key consideration, ensuring all users can engage with the website."
                      : index === 4
                      ? "I use project management tools like Trello, Asana, or Slack for communication and task tracking. We'll have regular check-ins to discuss progress, address concerns, and adjust timelines as needed."
                      : index === 5
                      ? "I focus on optimizing images, minimizing JavaScript and CSS, and leveraging caching strategies. I use performance tools like Lighthouse to identify and address bottlenecks, ensuring a fast and efficient user experience."
                      : index === 6
                      ? "I incorporate SEO best practices, such as semantic HTML, proper use of meta tags, and structured data where needed. I also focus on page load speed and mobile responsiveness, which are important factors for SEO."
                      : "I follow security best practices, including secure HTTP (HTTPS), input validation, and protection against common vulnerabilities like cross-site scripting (XSS) and cross-site request forgery (CSRF). I also keep dependencies and frameworks up-to-date to mitigate security risks."}
                  </p>
                )}
              </div>
            ))}
